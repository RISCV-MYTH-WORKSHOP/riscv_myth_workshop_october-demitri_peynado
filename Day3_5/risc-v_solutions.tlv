...
\TLV
   ...
   |cpu
      ...
      @1
         ...
         ?$imm_valid
            $imm[31:0] = $is_i_instr ? { {21{$instr[31]}},
                                          $instr[30:20] } :
                         $is_s_instr ? { {21{$instr[31]}},
                                          $instr[30:25],
                                          $instr[11:7] } :
                        $is_b_instr ? { {19{$instr[31]}},
                                          $instr[7],
                                          $instr[30:25],
                                          $instr[11:8],
                                          1'b0 } :
      ...
      @2   
         // Register file read
         //  (detect bypass required for hazards)
         $rs1_bypass_en = >>1$rf_wr_en && (>>1$rd == $rs1);
         $rs2_bypass_en = >>1$rf_wr_en && (>>1$rd == $rs2);
         //  (read the register file itself)
         $rf_rd_en1 = $rs1_valid;
         $rf_rd_en2 = $rs2_valid;
         $rf_rd_index1[4:0] = $rs1;
         $rf_rd_index2[4:0] = $rs2;
         $src1_value[31:0] = $rs1_bypass_en ? >>1$result :
                                              $rf_rd_data1;
